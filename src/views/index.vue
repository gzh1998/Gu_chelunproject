      ...
      <div class="indexright">
        <ul class="indexrightbox" @touchmove="touchmove">
          <li
            ...
            v-html="item.keys"
            v-show="item.keys"
            @click="gotopcar(index)"
          ></li>
        </ul>
      </div>
    </div>
    <Listcarright
      :class="isShow?'activeshow':'activehide'"
      v-bind:gochildren="list2"
      v-on:gofather="fatherdata"
    ></Listcarright>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    //时间
    setInterval(() => {
     this.car_time = Date.parse(new Date())
    }, 1000);
  },
  updated() {
    //  this.reload();
  ...
  methods: {
    ...
    getloading() {
      fetch("http://baojia-test.chelun.com/v2-car-getMasterBrandList.html")
        .then(res => {
          return res.json();
        })
        .then(body => {
